...
dnames = {}
dnames['air'] = "air.2m.mon.mean.nc"
dnames['tmin'] = "tmin.2m.mon.mean.nc"
dnames['tmax'] = "tmax.2m.mon.mean.nc"

param_name = sys.argv[2]

dataset = open_url(monthly_monolevel + dnames[param_name])
print dataset.keys() # should include param_name, e.g. 'air'
print dataset[param_name].shape
...
outfile = "africa_2m_%s_%d.csv" % (param_name,year)
csvout = csv.writer(open(outfile,'wb'))
csvout.writerow(["key","date","locn_id","location",param_name])
...
for r in range(nregions):
    ...
    if e == 0: e=360
    print "nw", n,w, "se", s,e
    a = dataset[param_name][interval,
                    (dataset.lat > s) & (dataset.lat < n),
                    (dataset.lon >= w) & (dataset.lon < e) ]
    #print "shape", a.shape
    data = numpy.round(a.array[:] * dataset[param_name].scale_factor + dataset[param_name].add_offset - 273.15, 1)
    ...
    for t in range(ntimes):
        ...
        for la in range(nlats):
            for lo in range(nlons):
                ...
                csvout.writerow([date_code(a.time[t]) + loc_code,
                        date_str(a.time[t]),loc_code,
